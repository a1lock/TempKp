...
const SetBuilder = () => {
  ...
  const handleSave = async () => {
    if (selectedItems.length === 0)
      return alert("добавьте хотя бы один предмет");
  ...
  const handleExportCSV = () => {
    ...
    const csvContent =
      ...
      selectedItems
        .map((i) => `"${i.market_name}","${i.weapon_type}",${i.price}`)
        .join("\n");
    ...
    selectedItems.length > 0
      ? (
          selectedItems.reduce(
            (sum, item) => sum + (item.min_float + item.max_float) / 2,
            0,
      ...
      <div>
        ...
        {editingId && (
          <div className="bg-blue-900/30 border border-blue-500 text-blue-300 p-3 rounded-lg mb-4 text-sm flex justify-between items-center">
            <span>
              Редактирование комплекта: <strong>{title}</strong> (сохранение
              обновит текущую запись в бд)
            </span>
            <button onClick={cancelEditing} className="underline text-xs">
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-12">
          ...
          <main className="flex-1 bg-[#1A1B23] p-6 rounded-xl border border-gray-800 flex flex-col gap-8">
            {["Knife", "Gloves", "Rifle"].map((cat) => {
              const catItems = selectedItems.filter(
                (i) => i.weapon_type === cat,
              );
              return (
                <div
                    ...
                    {Array(4)
                      ...
                      .map((_, idx) => {
                        ...
                        return item ? (
                          <div
                            key={idx}
                            className="bg-[#13141A] p-3 rounded-xl border border-gray-800 flex flex-col justify-between h-44"
                          >
                            <div
                            ...
                            {/* контейнер изображения предмета */}
                            ...
                            <div className="flex justify-between items-center mt-auto">
                              <span className="text-[#FF9408] text-xs font-bold">
                                {Number(item.price)} ₽
                              </span>
                              <button
                                onClick={() =>
          ...
          <aside className="w-full lg:w-96 bg-[#1A1B23] p-6 rounded-xl border border-gray-800 h-fit flex flex-col gap-6">
            ...
            <div>
              ...
              <div className="text-white text-sm font-medium">
                {selectedItems.length} из 12 макс.
              </div>
            </div>
            <div>
              <span className="text-gray-400 text-xs block mb-1">
            ...
            <div className="flex flex-col gap-2 mt-4">
              <button
                onClick={handleSave}
                className="w-full bg-[#FF9408] text-white py-3 rounded-lg font-bold hover:bg-orange-600 transition"
              >
                {editingId ? "Сохранить изменения" : "Сохранить набор"}
              </button>
              <button
                onClick={handleExportCSV}
                className="w-full bg-gray-700 text-white py-3 rounded-lg font-bold hover:bg-gray-600 transition"
              >
                Экспорт в CSV
              </button>
            </div>
        ...
        <section className="bg-[#1A1B23] p-6 rounded-xl border border-gray-800 mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {collections.map((col) => (
              <div
                key={col.id}
                className="flex justify-between items-center bg-[#13141A] p-4 rounded-lg border border-gray-800"
              >
                <span className="text-white font-medium">{col.title}</span>
                <div className="flex gap-4">
                  <button
                    onClick={() => handleEditLoad(col)}
            ...
            <div className="flex-1 overflow-y-auto flex flex-col gap-2 pr-2">
              {filteredModalItems.map((item) => (
                <button
                  ...
                  className="flex justify-between items-center p-2 bg-[#0F1014] hover:bg-gray-800 rounded-lg text-left text-sm text-white border border-gray-800 transition"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    {/* мини-превью картинки предмета */}
                    <div className="w-10 h-10 bg-[#13141A] rounded flex items-center justify-center p-1 shrink-0 border border-gray-800">
                      <img
                        src={`/img/${item.image_url}`}
                        alt={item.market_name}
                        className="h-full object-contain"
                  ...
                  <span className="text-[#FF9408] font-bold shrink-0 ml-4">
                    {Number(item.price)} ₽
                  </span>
                </button>
              ))}
