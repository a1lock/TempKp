...
const SetBuilder = () => {
  ...
  const [title, setTitle] = useState('Мой новый набор');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [activeCategory, setActiveCategory] = useState<string>('');
  const [modalSearch, setModalSearch] = useState('');

  // выносим функцию вверх для избежания ошибки hoisting
          ...
          <div className="bg-[#1A1B23] w-full max-w-lg rounded-xl p-6 border border-gray-800 flex flex-col max-h-[80vh]">
            ...
            <div className="flex-1 overflow-y-auto flex flex-col gap-2 pr-2">
              {filteredModalItems.length > 0 ? (
                ...
                <span className="text-gray-500 text-center text-sm my-4">нет доступных предметов</span>
              )}
            </div>
          </div>
        </div>
