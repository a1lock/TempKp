  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesType =
      selectedTypes.length > 0
        ? selectedTypes.includes(item.weapon_type)
        : true;
    const matchesExterior =
      selectedExteriors.length > 0
        ? selectedExteriors.includes(item.exterior)
        : true;
    return (
      matchesSearch &&
      matchesColor &&
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <aside className="w-full md:w-64 bg-[#1A1B23] p-6 rounded-xl border border-gray-800 h-fit shrink-0">
            ...
            <div className="mb-6">
              ...
              {[
                ...
                <label
                  key={type}
                  className="flex items-center gap-2 text-white text-xs mb-2 cursor-pointer"
                >
                  <input
                    type="checkbox"
              ...
              <div className="flex gap-1.5 flex-wrap">
                {[
                  ...
                  <button
                    key={color}
                    onClick={() =>
                      setColorFilter(color === colorFilter ? "" : color)
                    }
            ...
            <div className="mb-6">
              ...
              {[
                ...
                >
                  <input
                    type="checkbox"
                    checked={selectedExteriors.includes(ext)}
            ...
            <button
              ...
              className="text-gray-500 hover:text-white text-xs w-full text-center mt-4"
            >
              сбросить фильтры
            </button>
          </aside>
          ...
          <main className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map((item) => (
              <div
                key={item.id}
                className="bg-[#1A1B23] p-4 rounded-xl flex flex-col justify-between border border-gray-800 h-96"
              >
                <div
                  className="w-full h-1 rounded-t-xl mb-4"
                  ...
                  <div className="w-full h-full border border-gray-800 flex items-center justify-center relative bg-[#13141A]">
                    <div className="h-32 mb-4 flex items-center justify-center">
                      <img
                        ...
                        onError={(e) => {
                          // если картинка не найдена, покажется текстовая заглушка
